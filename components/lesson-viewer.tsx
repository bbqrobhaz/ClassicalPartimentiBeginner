...
export function LessonViewer({ lesson, onComplete, onNext, onPrevious, hasNext, hasPrevious }: LessonViewerProps) {
  ...
  const playExample = async (audioPattern: string[] | string[][]) => {
    try {
      for (const item of audioPattern) {
        // Check if item is an array (chord) or string (single note)
        if (Array.isArray(item)) {
          // Chord notation: array of note strings
          const playPromises = item.map((note) => {
            let noteName: string
            let octave: number
          ...
          // Legacy chord notation: string like "[C E G]"
